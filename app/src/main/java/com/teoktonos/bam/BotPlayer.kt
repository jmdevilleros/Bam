...
/**
 * Simple but effective AI player.
 * Uses a priority-based heuristic to choose the best legal move.
 */
class BotPlayer(
    game: BamGame,
    name: String,
    private val actionDelayMillis: Long = 500
) : BamPlayer(game, name) {
    ...
    private fun getValidMoves(): List<Pair<CardStack, CardStack>> {
        ...
        val targets = game.allStacks.asSequence()
            ...
            .filterNot { it.top()?.faceup == false }
            .filterNot { it in game.players.map { p -> p.zone.pull } }
